    ...
    try:
        if db_conn["type"] == "asyncpg":
            ...
            mentor_id = await conn.fetchval(
                "SELECT id FROM mentorship_relationships WHERE user_id = $1",
                mentor_user_id
            )
            if not mentor_id:
                return None
            ...
            result = await conn.fetchrow(
                ...
                (mentor_id, title, description, category, subcategory, price, duration, delivery_days, is_active)
                VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
                RETURNING id, mentor_id, title, description, category, subcategory, price, 
                         duration, delivery_days, is_active, total_orders, rating, created_at, updated_at
                """,
                mentor_id, service_data.title, service_data.description, service_data.category,
                service_data.subcategory, service_data.price, service_data.duration,
                service_data.delivery_days, service_data.is_active
            )
            ...
            result = client.table('services').insert({
                ...
                'delivery_days': service_data.delivery_days,
